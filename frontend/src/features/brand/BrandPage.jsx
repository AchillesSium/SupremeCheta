...
import { getAllBrands, deleteBrand } from './brandApi';
...
const BrandPage = () => {
  ...
  const queryArgs = useMemo(() => {
    return { page, limit, search: search.trim() };
  }, [page, limit, search]);
  ...
  const fetchBrands = async (signal) => {
    try {
      ...
      const data = await getAllBrands(queryArgs);
  ...
  const handleDelete = async (brand) => {
    ...
    try {
      ...
      const controller = new AbortController();
      await fetchBrands(controller.signal);
    } catch (e) {
